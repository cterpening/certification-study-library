...
The guide maps every July 27, 2026 objective bullet to a bidirectional packet-walk method, service and control comparisons, failure modes, two cross-domain scenarios, eight independent labs, and 24 original knowledge checks. The September 5 repair adds substantive requirements, configuration, validation, and troubleshooting coverage for Azure Network Adapter and Azure Extended Network in Section 3 and Lab 4. It cites 28 exact registered sources: 25 were reachable, while two O'Reilly pages and one Udemy page returned HTTP 403 to the automated checker. The official objective snapshot SHA-256 is `949dcb2d1b4bbde19b6f41b69fdf59cf33b52391d4b8010e2a46d6a0c94a98dd`.
...
| Manage virtual machines and containers | Section 4, Azure application scenario, and Labs 5–6 and 9; SSH Direct remains coverage-blocked |
...
The September 5 repair corrects Azure Extended Network's on-premises-subnet-to-Azure-VNet boundary and adds prerequisites, configuration, validation, troubleshooting, cleanup, and Lab 9 coverage for Azure Network Adapter and Extended Network. Source validation remains blocked for SSH Direct: Microsoft names the Linux-guest management objective but publishes no implementation source establishing configuration, authentication, validation, or troubleshooting. The record is therefore `review-required`, not a passed source validation. The remaining content has three scenarios, nine labs, and 24 checks. All 75 cited URLs are cataloged: 71 reachable, four access-blocked, and none missing. The official blueprint snapshot SHA-256 is `03c6f4e0c73f383ada8c4ddf43a91fa28ec513a732b40abae0a38b1b6ac4f7bc`.
...
| Manage virtual machines | Section 4, consolidated operating scenario, and Labs 4–5; SSH Direct remains coverage-blocked |
...
Six objective groups and the documented portions of the virtual-machine group map to operating dependencies, decisions, failure signals, and recovery proof. SSH Direct remains an explicit evidence and coverage exception: Microsoft names remote Linux-guest management in the blueprint, but no public implementation source establishes its configuration, identity, validation, or troubleshooting contract. The September 5 repair removed inferred Hyper-V-socket, OpenSSH, tunneling, and network-independence claims; the source-validation record is now blocked and the guide is `review-required`. The remaining content has three scenarios, eight labs, and 28 checks. All 77 cited URLs are cataloged: 72 reachable, five access-blocked, and none missing. The official blueprint snapshot SHA-256 is `ec584efe0ea08ae5ad6bbcef992c1b4a4e6b18826193ece06a76114757c8f65d`.
