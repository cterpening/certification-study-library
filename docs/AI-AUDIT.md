...
| **Recorded rubric-2 results** | **196** | **185** | **7** | **2** | **24 open / 19 closed** | **Same context** |

The original Terraform, Google Professional Agentic Architect, CCNA, Security+, Fortinet MSSP, NSE-8, CKS, PCPP-32-101, and AB-100 results remain historical where their bound guides changed. Their repaired or expanded versions now have current results: Security+, NSE-8, CKA, CKAD, CKS, PCPP-32-101, and AB-100 pass, while MSSP remains blocked only by its unpublished objective blueprint. Current rubric-2 coverage is 187 guides: 184 pass, one passes with notes, one requires a fix, and one is blocked. The remaining unaudited queue is 35 guides: 33 meet the preparation prerequisite, while AZ-800 and AZ-802 are unaudited source-gate blockers. Default manifests also continue to list MSSP as source-gate blocked even though its current blocked audit records the reason. A guide or objective change makes a completed result historical and returns that guide to the queue. Completed batches do not invalidate or erase earlier findings.
