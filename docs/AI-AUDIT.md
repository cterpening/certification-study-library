...
| **Recorded rubric-2 results** | **239** | **211** | **7** | **2** | **41 open / 30 closed** | **Same context** |

The original Terraform, Google Professional Agentic Architect, CCNA, Security+, Fortinet MSSP, NSE-8, CKS, PCPP-32-101, AB-100, and Oracle Foundations results remain historical where their bound guides changed. Their repaired or expanded versions now have current results: Security+, NSE-8, CKA, CKAD, CKS, PCPP-32-101, AB-100, Oracle Foundations, GitHub Enterprise Administration, and the repaired Fortinet guides pass, while MSSP remains blocked only by its unpublished objective blueprint. Current rubric-2 coverage is 220 guides: 210 pass, nine pass with notes, and one is blocked. The only unaudited guides are AZ-800 and AZ-802, both source-gate blockers. Default manifests also continue to list MSSP as source-gate blocked even though its current blocked audit records the reason. A guide or objective change makes a completed result historical and returns that guide to the queue. Completed batches do not invalidate or erase earlier findings.
