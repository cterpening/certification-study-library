...
EX200 establishes the Red Hat provider pattern: unweighted public task groups, performance evidence rather than question recall, safe break/fix labs, local-documentation fluency, and a mandatory runtime → persistent configuration → reboot → revalidation loop. EX280 applies the same evidence discipline to OpenShift reconciliation, declarative workloads, multi-tenancy, networking, Operators and application security while preserving its mixed-version warning. EX294 extends the pattern to reproducible fresh-host automation with declared execution environments/collections, idempotence, Git/VS Code workflow, navigator, roles, Vault and automated RHCSA outcomes.
