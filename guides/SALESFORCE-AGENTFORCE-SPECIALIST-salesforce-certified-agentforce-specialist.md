...
> **Source-access note — September 7, 2026; architecture link rechecked September 17:** Salesforce Help still emits a `Loading`/CSS wrapper to simple HTML checks, but an indexed text extraction exposed the complete current Spring ’26 guide: candidate boundaries, 60-plus-up-to-five/105-minute/72% contract, all six weights, and the objective list match this guide. The prior rendering concern is therefore cleared with that extraction recorded, not with a guess. Salesforce's current [Well-Architected Trust guidance](https://architect.salesforce.com/docs/architect/well-architected/guide/trust.html) replaces the old Secure route. It distinguishes customer agents running as dedicated, least-privilege Agent Users from employee agents using the logged-in user's context; inspect the execution and external-system security context of every action. This supports secure labs; the exam guide remains scope authority.
...
| [Salesforce Well-Architected — Trust](https://architect.salesforce.com/docs/architect/well-architected/guide/trust.html) | Public | 2–4 hr selected plus threat model | Agent-user identity, least privilege, action execution context, data classification, sharing, and integration security; architecture practice, not extra exam scope |
